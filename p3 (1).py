...
##        cursor.execute(query)
    ...
    def confirmation(self):
        ...
        query2 = "INSERT INTO RESERVES(ReservationID, Train_Number, Class, Departure_Date, Passenger_Name, Number_of_Bags, Departs_From,\
            Arrives_At, Total_Cost) VALUES ('%d', '%d', '%d', '%Y-%m-%d', '', 0, '%s', '%s', '%f')"% (self.newReservationID, self.trainChosen, self.classChosen, self.date.get(), self.results1[0][4], self.results1[0][5], self.price)
        cursor.execute(query2)
